...
describe("resolveOutputPath — security", () => {
  it("rejects path traversal with ../", () => {
    expect(() =>
      resolveOutputPath(`${BASE}/sub/file.srt`, "../escape/{name}.ass", "PQ")
    ).toThrow();
  });
  ...
  it("rejects empty template", () => {
    expect(() =>
      resolveOutputPath(`${BASE}/subtitle.srt`, "", "PQ")
    ).toThrow();
  });

  it("rejects Windows reserved name CON", () => {
    expect(() =>
      resolveOutputPath(`${BASE}/subtitle.srt`, "CON.ass", "PQ")
    ).toThrow(/reserved/i);
  });

  it("rejects Windows reserved name with dollar sign (CONIN$)", () => {
    expect(() =>
      resolveOutputPath(`${BASE}/subtitle.srt`, "CONIN$.ass", "PQ")
    ).toThrow(/reserved/i);
  });
  ...
  it("rejects non-absolute input paths", () => {
    expect(() =>
      resolveOutputPath("subtitle.srt", "{name}.hdr.ass", "PQ")
    ).toThrow(/absolute/i);
  });

  it("rejects overwriting source file", () => {
    expect(() =>
      resolveOutputPath(`${BASE}/subtitle.ass`, "{name}.ass", "PQ")
    ).toThrow(/overwrite|same/i);
  });
});
